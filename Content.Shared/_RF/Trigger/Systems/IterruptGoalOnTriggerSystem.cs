...
public sealed class IterruptGoalOnTriggerSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<IterruptGoalOnTriggerComponent, TriggerEvent>(OnTrigger);
    }

    public void OnTrigger(Entity<IterruptGoalOnTriggerComponent> ent, ref TriggerEvent ev)
    {
        if (ev.Key == null
            || !ent.Comp.Goals.TryGetValue(ev.Key, out var goals)
            || !_utilityAi.TryGetCurrentGoal(ent.Owner, out var current)
            || !goals.Contains(current)
